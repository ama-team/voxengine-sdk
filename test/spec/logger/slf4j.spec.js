...
describe('/logger', function () {
    describe('/slf4j.js', function () {
        ...
        describe('.Factory', function () {
            it('should allow threshold change', function () {
                var factory = new slf4j.Factory(Level.Info),
                    loggerA = factory.create(),
                    loggerB;

                factory.setThreshold(Level.Error);
                loggerB = factory.create();

                loggerA.getThreshold().should.eq(Level.Info);
                loggerB.getThreshold().should.eq(Level.Error);
            });

            it('should allow writer change', function () {
                var writerA = {write: function () {}},
                    writerB = {write: function () {}},
                    factory = new slf4j.Factory(Level.Info, writerA),
                    loggerA = factory.create(),
                    loggerB;

                factory.setWriter(writerB);
                loggerB = factory.create();

                loggerA.getWriter().should.eq(writerA);
                loggerB.getWriter().should.eq(writerB);
            });
        })
    });
});