...
function RestClient (settings, transport) {
  ...
  var client
  var clientOpts = basic.getDefaults()
  var logger
  ...
  Object.keys(opts).forEach(function (key) {
    if (key in settings) {
      clientOpts[key] = opts[key] = settings[key]
    }
  })
  client = settings.client || new basic.Client(clientOpts, transport)
  ...
  this.exists = function (route, query, headers) {
    return request(Method.Head, route, null, query, headers)
  ...
  this.get = function (route, query, headers) {
    return request(Method.Get, route, null, query, headers)
      .then(function (response) {
  ...
  Object.keys(methods).forEach(function (method) {
    self[method] = function (resource, payload, headers, query) {
      logger.debug('Executing `{} {}` request', method, resource)
