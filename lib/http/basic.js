...
function BasicHttpClient (settings, transport) {
  ...
  function execute (request) {
    ...
    if (!request.method) {
      ...
      return Promise.reject(new Commons.InvalidConfigurationException(message))
    }
  ...
  function executionLoop (request, attempt) {
    ...
    var headers = Headers.override(setting('headers', {}), request.headers)
    ...
    return transport(url, opts).then(function (raw) {
      ...
      if (toThrow) performThrow(status, request, response, raw.error)
