 ...
 *
 * @property {(Slf4j|null)} instance
 * @property {(Context|null)} context
 * @property {(String|null)} name
 */

/**
 * @param {(LoggerOptions|null)} [options]
 * @param {(String|null)} [name] Default name to use in case options don't include it
 * @param {(Context|null)} [context] Default context
 * @return Slf4j
 */
Slf4j.assemble = function (options, name, context) {
  options = options || {}
  context = options.context || context || Slf4j
  if (options.instance) {
    return options.instance
  }
  return context.create(options.name || name)
}
