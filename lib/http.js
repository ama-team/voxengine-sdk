 ...
 * @property {LoggerInterface|undefined} logger Sink to send log messages into, default: just empty callback
 */
 ...
 */
function RestClient(netHttpRequestAsync, settings) {
    ...
    function assembleHeaders(headers) {
        return Object.keys(headers).reduce(function (carrier, key) {
            return carrier.concat(headers[key].map(function (value) {
                return key + ': ' + value;
            }));
        }, []);
    }
    ...
    function missingResourceHandler(response, request, attempt) {
        logger.log(LogLevel.WARN,'Got missing resource response while executing request {} on attempt #{}: {}', request,
            attempt, response);
        throw {
    ...
    function redirectHandler(response, request, attempt) {
        ...
        if (!location) {
            logger.log(LogLevel.WARN, 'Invalid redirect response received ({}), retrying', response);
            return tryPerformRequest(request, attempt + 1);
        }
        logger.log(LogLevel.INFO, 'Received redirect response, trying location {}', location);
        var nextRequest = {
    ...
    function clientErrorHandler(response, request, attempt) {
        ...
        logger.log(LogLevel.ERROR, 'Failed to execute request {}, attempt #{}: got client error ({})', request, attempt,
            response);
        return tryPerformRequest(request, attempt + 1);
    }
    ...
    function serverErrorHandler(response, request, attempt) {
        ...
        logger.log(LogLevel.WARN, 'Failed to execute request {}, attempt #{}: got server error ({})', request, attempt,
            response);
        return tryPerformRequest(request, attempt + 1);
    }
    ...
    function networkErrorHandler(response, request, attempt) {
        ...
        logger.log(LogLevel.WARN, 'Failed to execute request {}, attempt #{}: got connection error', request, attempt);
        return tryPerformRequest(request, attempt + 1);
    }
    ...
    function tryPerformRequest(request, attempt) {
        ...
        logger.log(LogLevel.DEBUG, 'Trying to execute request {}', request);
        nonStandardMethod = [HttpMethod.Get, HttpMethod.Post].indexOf(request.method) === -1;
        ...
        options.method = request.method === HttpMethod.Get ? HttpMethod.Get : HttpMethod.Post;
        options.headers = [];
        ...
        return netHttpRequestAsync(url, options)
     ...
     * @return {Promise.<HttpResponse>} Promise for response
     */
    ...
    this.get = function (route, query, headers) {
        return this.request(HttpMethod.Get, route, null, query, headers);
    };

    //noinspection JSUnusedGlobalSymbols
    this.create = function (route, payload, headers) {
        return this.request(HttpMethod.Post, route, payload, null, headers);
    };

    //noinspection JSUnusedGlobalSymbols
    this.set = function (route, payload, headers) {
        return this.request(HttpMethod.Put, route, payload, null, headers);
    };

    //noinspection JSUnusedGlobalSymbols
    this.delete = function (route, payload, headers) {
        return this.request(HttpMethod.Delete, route, payload, null, headers);
    }
}
